...
export const ReservaDetalles = ({ formData, onFormChange, onDateChange, onServiceToggle }) => {
  ...
  const handleServiceToggle = (servicio) => {
    ...
    if (formData.serviciosAdicionales.some((s) => s.id === id)) {
      const updatedServices = formData.serviciosAdicionales.filter((s) => s.id !== id);
      onServiceToggle(updatedServices);
    } else {
      onServiceToggle([...formData.serviciosAdicionales, { id, nombre, precio }]);
    }
  };
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) => {
                const [year, month, day] = e.target.value.split('-').map(Number);
                const localDate = new Date(year, month - 1, day); // Month is 0-indexed
                onDateChange(localDate, "end");
              }}
              className="w-full px-3 py-2 border rounded-md"
              min={
                formData.fechaInicio
                  ? format(new Date(formData.fechaInicio.getTime() + 86400000), "yyyy-MM-dd")
            ...
            servicios
              ...
              .map((servicio) => (
                <div key={servicio.id} className="flex items-start space-x-3 border rounded-md p-3">
                  <input
                    type="checkbox"
                    checked={formData.serviciosAdicionales.some((s) => s.id === servicio.id)}
