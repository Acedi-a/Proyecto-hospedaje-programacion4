...
import MiniHabitacionCard from "../../components/user/MiniCardHabitacionCelia";
...
export const Celia = () => {
  ...
  useEffect(() => {
    const auxCacheIA = localStorage.getItem("habitacionesCache");
    ...
    try {
      const chatBody = {
        ...
        systemInstruction: {
          role: "system",
          parts: [{
            text: `Eres Celia, una asistente virtual amigable de un hospedaje algo juguetona. Eres experta en seguir las ordenes y expera en responder en formato JSON, responderas únicamente en formato JSON válido, estructurado exactamente con las siguientes claves:
...
${JSON.stringify(cacheIA, null, 2)}`
      ...
      let geminiReplyJSON = null;
      let finalReplyText = "Lo siento, hubo un error al procesar la respuesta de la IA. 🤔";

      try {
        //geminiReplyJSON = JSON.parse(geminiReplyRawString);
        console.log("RESPUESTA FINAL:", geminiReplyRawString);
        ...
        if (geminiReplyJSON.Habitacion) {
          ...
          finalReplyText += "\nDetalles de la Habitación Sugerida:\n" + JSON.stringify(geminiReplyJSON.Habitacion, null, 2);
        }
      } catch (error) {
        console.error("Error al parsear JSON:", error, geminiReplyRawString);
        finalReplyText = geminiReplyRawString;
      }
        ...
        <div className="bg-emerald-100 w-full max-w-2xl p-6 rounded-2xl shadow-lg flex flex-col flex-grow">
          ...
          >
            {messages.map((msg, index) => (
              ...
              >
                <div
                  className={`px-4 py-2 rounded-2xl max-w-xs md:max-w-md break-words ${msg.from === "user"
